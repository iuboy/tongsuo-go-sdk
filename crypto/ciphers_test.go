...
func TestBadInputs(t *testing.T) {
	...
	c, err := crypto.GetCipherByName("AES-128-CBC")
	if err != nil {
...
func doEncryption(key, iv, aad, plaintext []byte, blocksize, bufsize int) ([]byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Failed to add authenticated data: %w", err)
	}
	...
	for plainb.Len() > 0 {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("Failed to perform an encryption: %w", err)
		}
	...
	tag, err := ectx.GetTag()
	if err != nil {
		return nil, nil, fmt.Errorf("Failed to get GCM tag: %w", err)
	}
...
func doDecryption(key, iv, aad, ciphertext, tag []byte, blocksize, bufsize int) ([]byte, error) {
	...
	for aadbuf.Len() > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed to add authenticated data: %w", err)
		}
	}
	...
	for cipherb.Len() > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed to perform a decryption: %w", err)
		}
	...
	err = dctx.SetTag(tag)
	if err != nil {
		return nil, fmt.Errorf("Failed to set expected GCM tag: %w", err)
	}
...
func checkEqual(t *testing.T, output []byte, original string) {
	...
	if outputStr != original {
		t.Fatalf("output != original! %#v != %#v", outputStr, original)
	}
}
...
func TestGCM(t *testing.T) {
	...
	blocksizesToTest := []int{256, 192, 128}
	...
	for _, bsize := range blocksizesToTest {
		subkey := key[:bsize/8]
		...
		plaintextOut, err := doDecryption(subkey, iv, aad, ciphertext, tag,
			bsize, bufsize)
		if err != nil {
...
func TestBadTag(t *testing.T) {
	...
	_, err = doDecryption(key, iv, nil, ciphertext, tag, 128, 129)
	if err == nil {
...
func TestBadCiphertext(t *testing.T) {
	...
	_, err = doDecryption(key, iv, aad, ciphertext, tag, 192, 192)
	if err == nil {
...
func TestBadAAD(t *testing.T) {
	...
	_, err = doDecryption(key, iv, aad, ciphertext, tag, 256, 256)
	if err == nil {
